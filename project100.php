...
<div class="container project-wrapper">
    ...
    if (isset($_GET['project_id'])) {
        ...
        if (isset($_GET['invite']) && isset($_GET['verify'])) {
            ...
            $check_user_inproject = "SELECT COUNT(*) AS user_count FROM ProjectUsers WHERE user_id = $user_id AND project_id = $project_id";

            // Execute the SQL query
            $check_user_existsin_project_result = mysqli_query($connection, $check_user_inproject);

            if ($check_user_existsin_project_result) {
                $row = $check_user_existsin_project_result->fetch_assoc();
                ...
                ORDER BY ProjectUsers.is_projectowner DESC, Users.username";
                    ...
                    // The user does not exist in the project, so exit the script
                    echo '<h1 class="warning-style">No project found or you are unable to view this project.</h1>';
                    exit();
                }
            }
        }
