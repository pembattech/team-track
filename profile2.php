                ...
                <div class="user-tasks">
                    ...
                    if (mysqli_num_rows($incomplete_tasks) > 0) {
                        ...
                        while ($task = mysqli_fetch_assoc($incomplete_tasks)) {
                            echo "<tr class='task-row $displayClass'>";
                            echo "<td class='task-cell tooltip' data-task-id='" . $task['task_id'] . "'>";
                            echo $task['task_name'];
                            echo "</td>";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    taskRows.forEach((row) => {
        ...
        const taskId = taskCell.getAttribute("data-task-id");
        
        taskCell.addEventListener("mouseover", () => {
            fetchTaskDescription(taskId);
        });
        
        taskCell.addEventListener("mouseout", () => {
            removeTooltip();
        });
    });
});

function fetchTaskDescription(taskId) {
    fetch(`fetch_task_description.php?task_id=${taskId}`)
...
function displayTooltip(description) {
    const tooltip = document.createElement("div");
    tooltip.className = "tooltip"; // Apply your tooltip class here
    tooltip.innerHTML = `<div class="tooltiptext">${description}</div>`;
